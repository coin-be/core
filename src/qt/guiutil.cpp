...
{
    // Convert coinbe:// to coinbe:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("coinbe://"))
    {
        uri.replace(0, 12, "coinbe:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CoinBe.lnk";
}
...
{
    return GetAutostartDir() / "coinbe.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CoinBe-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  coinbe-qt [" + tr("command-line options") + "]                     " + "\n";
